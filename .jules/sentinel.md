...
## 2026-01-14 - [Timing Attack Prevention in Webhook Verification]
**Vulnerability:** The WhatsApp webhook verification in `main.py` used `==` to compare the `hub.verify_token`.
**Learning:** Webhook tokens are shared secrets and subject to timing attacks just like API keys.
**Prevention:** Use `secrets.compare_digest()` for all token comparisons, including webhook verification handshakes.
