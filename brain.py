...
def process_message(user_text: str, phone_number: str, message_id: Optional[str] = None, image_data: Optional[bytes] = None, audio_data: Optional[bytes] = None) -> Union[str, bytes, None]:
    ...
    # ⚡ Performance: Paralelizamos Análisis de Intención y Búsqueda Vectorial
    # Lanzamos el análisis de intención
    intent_future = _executor.submit(_analyze_tone_and_intent, user_text, history)

    # Lanzamos la búsqueda vectorial (Optimista) asumiendo que podría ser una consulta de ventas
    search_query = user_text
    if image_context:
        search_query += f" {image_context}"
    search_future = _executor.submit(_search_cars, search_query)

    # Esperamos el resultado del análisis de intención primero para decidir
    ...
    try:
        ...
        if intent == "FEEDBACK_NEG":
            ...
            # Obtenemos el resultado de la búsqueda que ya se estaba ejecutando
